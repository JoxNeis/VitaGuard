...
@section('content')
    <div class="container mt-4 mb-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4><i class="bi bi-pencil-square text-primary"></i> Edit Data Member</h4>
                <p class="text-muted mb-0">Perbarui informasi akun dan profil pasien/member.</p>
            </div>
    ...
    <script>
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            districtSelect.append('<option value="" disabled>-- Pilih Wilayah --</option>');
            ...
            function loadMemberData() {
                $.ajax({
                    url: `/api/admin/members/${targetUsername}/edit-data`,
                    method: 'GET',
                    success: function (response) {
                        if (response.success) {
                            ...
                            $('#middle_name').val(member.middle_name);
                            $('#last_name').val(member.last_name);
                            $('#gender').val(member.gender);
                            $('#date_of_birth').val(member.date_of_birth);
                            $('#address').val(member.address);                            
                            $('#district_id').val(member.district_id);
                        }
                    },
                    error: function () {
                        alert("Gagal mengambil data member dari server.");
                    }
                });
            }
            ...
            $('#form-edit-member').on('submit', function (e) {
                ...
                let formData = new FormData($('#form-edit-member')[0]);

                $.ajax({
                    url: `/api/admin/members/${targetUsername}/update`,
                    method: 'POST',
                    processData: false,
                    contentType: false,
                    data: formData,
